(function () {
  ...
  let lastProcessedCharAvatar: string | null = null; // 用于跟踪上一个处理过的角色，防止重复触发
  ...
  async function getStoredConfigs(): Promise<Record<string, any>> {
    let worldbookEntries: any[];
    try {
  ...
  async function setStoredConfigs(configsObject: Record<string, any>): Promise<void> {
    try {
      const nameCounts: Record<string, number> = {};
      const entries = Object.values(configsObject).map((config: any) => {
  ...
  async function showRegexBindingPopup(configId: string) {
    ...
    try {
      ...
      $('#regex-bind-save').on('click', async () => {
        ...
        const otherConfigIds = Object.keys(cfgs).filter(
          id => id !== configId && cfgs[id].presetName === currentPresetName,
        );
  ...
  function showNewEntriesPopup(newEntries: any[], promptIdToNameMap: Map<string, string>) {
  ...
  function showConfigSelectionPopup(configs: any[], charName: string) {
    return new Promise(resolve => {
  ...
  async function showRegexExportSelectionPopup(boundRegexes: any[]) {
    return new Promise(resolve => {
  ...
  async function renameConfig(configId: string) {
    const configs = await getStoredConfigs();
    ...
    if (newName && (newName as string).trim() !== '') {
      configs[configId].name = (newName as string).trim();
      await setStoredConfigs(configs);
      toastr.success(`配置已从 \"${oldName}\" 重命名为 \"${(newName as string).trim()}\"。`);
      await renderConfigsList();
    } else {
      toastr.info('重命名操作已取消。');
    }
  }
  async function updateConfig(configId: string) {
    try {
      const configs = await getStoredConfigs();
      ...
      if (updateRegexChoice === '1') {
        ...
        (configToSave as any).regexStates = newRegexStates;
        toastr.info('已同步更新正则状态。');
      }

      const oldStateIds = new Set(oldConfig.states.map((s: any) => s.id));
      const newEntries = (configToSave.states as any[]).filter(s => !oldStateIds.has(s.id));
      ...
      if (newEntries.length > 0) {
        ...
        if (userChoices !== null) {
          const choicesMap = new Map(userChoices.map((choice: any) => [choice.id, choice.enabled]));
          (configToSave.states as any[]).forEach(state => {
      ...
      await setStoredConfigs(configs);
      toastr.success(`配置 \"${(configToSave as any).name}\" 已更新。`);
      await renderConfigsList();
    } catch (error) {
  ...
  async function saveCurrentConfig() {
    ...
    const configName = (nameInput.val() as string).trim();
    if (!configName) {
    ...
    try {
      ...
      const configs = await getStoredConfigs();
      (configs as any)[configToSave.id] = configToSave;
      await setStoredConfigs(configs);
  ...
  async function loadConfig(configId: string, shouldToggleUI = true) {
    try {
      const configs = await getStoredConfigs();
      ...
      const statesMap = new Map(promptStates.map((s: any) => [s.id, s.enabled]));
      ...
      if (configToLoad.regexStates && Array.isArray(configToLoad.regexStates)) {
        const statesToApply = new Map(configToLoad.regexStates.map((r: any) => [r.id, r.enabled]));
  ...
  async function exportConfig(configId: string) {
    try {
      const configs = await getStoredConfigs();
      ...
      if (presetName && TavernHelper.getPresetNames().includes(presetName)) {
        const includePresetChoice = await triggerSlash(
          `/popup okButton="是" cancelButton="否" result=true "此配置关联了预设 \"${presetName}\"。是否要将预设文件本身一起打包导出？"`,
        );
        if (includePresetChoice === '1') {
          const presetData = TavernHelper.getPreset(presetName);
          if (presetData) {
            (presetData as any).name = presetName;
      ...
      if (configData.regexStates && (configData.regexStates as any[]).length > 0) {
        const userChoice = await triggerSlash(
          `/popup okButton=\"是\" cancelButton=\"否\" result=true \"此配置绑定了正则。是否选择要一起导出的正则？\"`,
        );
        if (userChoice === '1') {
          const boundRegexIds = new Set((configData.regexStates as any[]).map((r: any) => r.id));
          const allGlobalRegexes = await TavernHelper.getTavernRegexes({ scope: 'global' });
          const boundRegexes = (allGlobalRegexes as any[]).filter(r => boundRegexIds.has(r.id));

          const selectedRegexes = await showRegexExportSelectionPopup(boundRegexes);
          if (selectedRegexes) {
            exportBundle.regexData = selectedRegexes as any[];
            toastr.info(`已将 ${(selectedRegexes as any[]).length} 条正则打包导出。`);
          } else {
            toastr.info('已取消导出正则。');
          }
        }
      ...
      let userFileName = (await triggerSlash(
        `/input default=\"${defaultFileName}\" \"请输入导出的文件名（无需后缀）\"`,
      )) as string;
  ...
  async function handleFileImport(event: Event) {
    ...
    try {
      ...
      if (data.type === 'MiaoMiaoPresetBundle' && data.version === 1) {
        ...
        const existingNames = Object.values(existingConfigs).map((c: any) => c.name);
        ...
        if (presetData) {
          const presetName = (presetData as any).name;
          if (presetName && !TavernHelper.getPresetNames().includes(presetName)) {
            const importPresetChoice = await triggerSlash(
              `/popup okButton="是" cancelButton="否" result=true "文件包含预设 \"${presetName}\"，是否要导入此预设？"`,
            );
        ...
        if (regexData && Array.isArray(regexData) && regexData.length > 0) {
          const importRegexChoice = await triggerSlash(
            `/popup okButton="是" cancelButton="否" result=true "文件包含 ${regexData.length} 条正则，是否要导入这些正则？"`,
          );
  ...
  async function deleteConfig(configId: string) {
    ...
    const confirmDelete = await triggerSlash(
      `/popup okButton="删除" cancelButton="取消" result=true "确定要删除配置 \"${configToDelete.name}\" 吗？此操作不可撤销。"`,
    );
    if (confirmDelete === '1') {
  ...
  async function importRegexLogic(regexToImport) {
    try {
      ...
      const existingNames = new Set(existingRegexes.map((r: any) => r.script_name));
      const existingIds = new Set(existingRegexes.map((r: any) => r.id));
      ...
      for (const regex of regexToImport) {
        if (existingIds.has(regex.id)) {
          ...
          const duplicateChoice = await triggerSlash(
            `/popup okButton="跳过" cancelButton="覆盖" result=true "正则 \"${regex.script_name}\" 已存在，是否跳过？"`,
          );
  ...
  async function showBatchExportPopup() {
    const configs = await getStoredConfigs();
    const configList = Object.values(configs as any);
    ...
    $('#batch-export-confirm').on('click', async () => {
      ...
      const selectedConfigs = configList.filter((c: any) => selectedIds.has(c.id));
      $(`#${popupId}`).remove();
  ...
  async function batchExportConfigs(selectedConfigs) {
    ...
    try {
      let userRemark = '';
      const addRemarkChoice = await triggerSlash(
        `/popup okButton="是" cancelButton="否" result=true "是否要为此批量导出添加备注信息？"`,
      );
      if (addRemarkChoice === '1') {
        userRemark = (await triggerSlash(
          `/input multiline=true placeholder="请输入备注，例如导出时间、用途等..." "添加备注"`,
        )) as string;
      }
      ...
      const presetNames = [...new Set(selectedConfigs.map((c: any) => c.presetName).filter(Boolean))];
      if (presetNames.length > 0) {
        const includePresetsChoice = await triggerSlash(
          `/popup okButton="是" cancelButton="否" result=true "是否要将关联的预设文件一起打包导出？"`,
        );
        if (includePresetsChoice === '1') {
          const presetDataMap: any = {};
      ...
      selectedConfigs.forEach((config: any) => {
        if (config.regexStates) {
          config.regexStates.forEach((r: any) => allRegexIds.add(r.id));
        }
      });

      if (allRegexIds.size > 0) {
        const includeRegexChoice = await triggerSlash(
          `/popup okButton="是" cancelButton="否" result=true "是否要将绑定的正则一起导出？"`,
        );
        if (includeRegexChoice === '1') {
          const allGlobalRegexes = await TavernHelper.getTavernRegexes({ scope: 'global' });
          const boundRegexes = (allGlobalRegexes as any[]).filter(r => allRegexIds.has(r.id));
  ...
  async function startBatchImportFlow(configsToImport) {
    ...
    try {
      const existingConfigs = await getStoredConfigs();
      const existingNames = new Set(Object.values(existingConfigs).map((c: any) => c.name));
      ...
      for (const config of configsToImport) {
        ...
        if (finalName !== config.name) {
          const renameChoice = await triggerSlash(
            `/popup okButton="重命名" cancelButton="跳过" result=true "配置 \"${config.name}\" 已存在，是否重命名为 \"${finalName}\"？"`,
          );
          if (renameChoice === '1') {
      ...
      if (configsToAdd.length > 0) {
        configsToAdd.forEach(config => {
          ...
          (existingConfigs as any)[config.id] = config;
        });
        await setStoredConfigs(existingConfigs);
  ...
  async function batchDeleteConfigs(configIds) {
    ...
    const configs = await getStoredConfigs();
    const configsToDelete = configIds.map(id => (configs as any)[id]).filter(Boolean);
    ...
    const confirmDelete = await triggerSlash(
      `/popup okButton="删除" cancelButton="取消" result=true "确定要删除 ${configsToDelete.length} 个配置吗？此操作不可撤销。"`,
    );
  ...
  async function onChatChanged() {
    try {
      const charData = await TavernHelper.getCharData('current');
      if (!charData || !charData.avatar) return;

      const currentCharAvatar = charData.avatar;
      if (lastProcessedCharAvatar === currentCharAvatar) return;

      lastProcessedCharAvatar = currentCharAvatar;
      const configs = await getStoredConfigs();
      const boundConfigs = Object.values(configs as any).filter(
        (config: any) => config.boundCharAvatar === currentCharAvatar,
      );
      ...
      if (boundConfigs.length === 1) {
        ...
        const loadChoice = await triggerSlash(
          `/popup okButton="加载" cancelButton="取消" result=true "检测到角色 \"${charData.name}\" 绑定了配置 \"${config.name}\"，是否加载？"`,
        );
        if (loadChoice === '1') {
...
$(() => {
  ...
  type PromptState = { id: string; enabled: boolean; name: string };
  type GroupDef = {
    id: string;
    name: string;
  ...
  async function readAllPresetGroups(): Promise<Record<string, PresetGroups>> {
    try {
      const entries = await TavernHelper.getWorldbook(GROUPS_LOREBOOK_NAME);
      const result: Record<string, PresetGroups> = {};
      for (const e of entries) {
        try {
          const obj = JSON.parse(e.content) as PresetGroups;
  ...
  async function writeAllPresetGroups(map: Record<string, PresetGroups>) {
  ...
  async function getCurrentPresetName(): Promise<string> {
    return TavernHelper.getLoadedPresetName();
  }

  function getCurrentPromptStates(): PromptState[] {
    const preset = TavernHelper.getPreset('in_use');
  ...
  function renderPromptsList(prompts: PromptState[]) {
    ...
    prompts.forEach((p, idx) => {
      ...
      row.append(cb);
      row.append(
        $(`<div style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"></div>`).text(p.name),
      );
      row.on('click', (e: JQuery.ClickEvent) => {
        const shift = (e as JQuery.ClickEvent).shiftKey;
  ...
  async function renderGroupsList(groups: GroupDef[], prompts: PromptState[]) {
    ...
    const idToIndex = new Map(prompts.map((p, i) => [p.id, i] as const));
    groups.forEach(g => {
      ...
      rename.on('click', async () => {
        ...
        const item = cur.groups.find(x => x.id === g.id);
        if (!item) return;
        item.name = (newName as string).trim();
        await writeAllPresetGroups(map);
        render();
        applyGroupsToPage();
      });
      ...
      del.on('click', async () => {
        const ok = await triggerSlash(
          `/popup okButton="删除" cancelButton="取消" result=true "确认删除分组：${g.name}"`,
        );
        if (ok !== '1') return;
        const map = await readAllPresetGroups();
        const preset = await getCurrentPresetName();
  ...
  async function createGroupFromSelection() {
    ...
    const nameInput = $(`#${UI_ID}-group-name`).val() as string;
  ...
  function findPromptListNodes(): JQuery<HTMLElement> {
    const candidates = getSelectorCandidates();
    for (const sel of candidates) {
      const nodes = $(sel as string);
      if (nodes.length >= 2) return nodes as any;
    }
    return $();
  }

  function getSelectorCandidates(): string[] {
    // 1) 用户为当前预设自定义的选择器（优先）
    // 2) 预置候选
    ...
    const custom =
      ((window as unknown as { __presetGroupsSelectors?: Record<string, string[]> }).__presetGroupsSelectors || {})[
        preset
      ] ?? [];
  ...
  async function openSelectorDialog() {
    ...
    const readLines = (): string[] => {
      return ($(`#${popupId}-input`).val() as string)
        .split(/\r?\n/)
        .map(s => s.trim())
        .filter(Boolean);
    };
    $(`#${popupId}-test`).on('click', () => {
      ...
      let matched = 0;
      for (const sel of lines as string[]) {
        try {
          const nodes = $(sel as string);
    ...
    $(`#${popupId}-save`).on('click', async () => {
      ...
      (window as unknown as { __presetGroupsSelectors?: Record<string, string[]> }).__presetGroupsSelectors =
        (window as unknown as { __presetGroupsSelectors?: Record<string, string[]> }).__presetGroupsSelectors || {};
      (
        (window as unknown as { __presetGroupsSelectors?: Record<string, string[]> }).__presetGroupsSelectors as Record<
          string,
          string[]
        >
      )[preset] = lines;
  ...
  function mapPromptsToNodes(prompts: PromptState[]) {
    const nodes = findPromptListNodes();
    if (nodes.length === 0) return { nodes, map: [] as number[] };
  ...
  async function applyGroupsToPage() {
    ...
    if (nodes.length === 0) {
      if (!appliedOnce) toastr.info('未找到预设条目列表的DOM，无法折叠。可在不同主题下再试。');
      return;
    }
    ...
    for (const g of cur.groups) {
      ...
      if (startNodeIdx < 0 || endNodeIdx < 0) continue;
      const startEl = nodes.get(startNodeIdx) as HTMLElement | undefined;
      const endEl = nodes.get(endNodeIdx) as HTMLElement | undefined;
      ...
      let curNode: JQuery<HTMLElement> = startNode;
      const toMove: JQuery<HTMLElement>[] = [];
      while (true) {
        const n: JQuery<HTMLElement> = curNode;
        toMove.push(n);
        if (n.is(endNode)) break;
        const next = n.next();
        if (next.length === 0) break;
        curNode = next as JQuery<HTMLElement>;
      }
      toMove.forEach(n => {
        (n as any).attr('data-preset-group-item', '');
      });
      ...
      header.on('click', async () => {
        ...
        if (cur2) {
          ...
          if (it) it.collapsed = g.collapsed;
  ...
  async function exportGroups() {
    ...
    const includeConfig = await triggerSlash(
      `/popup okButton="包含" cancelButton="仅分组" result=true "是否在导出中包含当前预设的条目开关配置？"`,
    );
    const bundle: any = {
    ...
    input.on('change', async e => {
      try {
        ...
        if (data.presetName && data.presetName !== preset) {
          const ok = await triggerSlash(
            `/popup okButton="应用" cancelButton="取消" result=true "导入文件来自预设：${data.presetName}，是否应用到当前预设：${preset}？"`,
          );
          if (ok !== '1') return;
        }
        const applyGroups = await triggerSlash(
          `/popup okButton=\"应用分组\" cancelButton=\"跳过\" result=true \"是否应用文件中的分组设置？\"`,
        );
        const applyConfig = data.config
          ? await triggerSlash(
              `/popup okButton="应用配置" cancelButton="跳过" result=true "文件包含条目开关配置，是否应用？"`,
            )
          : '0';
        if (applyGroups === '1') {
          const all = await readAllPresetGroups();
          all[preset] = {
            presetName: preset,
            groups: (data.groups as GroupDef[]).map(g => ({ ...g, id: genId() })),
        ...
        if (applyConfig === '1' && data.config && Array.isArray(data.config.states)) {
          const states = new Map<string, boolean>(data.config.states.map((s: any) => [s.id, s.enabled]));
          await TavernHelper.updatePresetWith('in_use', presetObj => {
            [...presetObj.prompts, ...presetObj.prompts_unused].forEach(p => {
              if (states.has(p.id)) p.enabled = states.get(p.id)!;
            });
            return presetObj;
          });
  ...
  async function init() {
    ...
    (window as unknown as { PresetGroups_openUI?: () => void }).PresetGroups_openUI = () => {
      ensureUI();
      const ui = $(`#${UI_ID}`);
      if (!ui.is(':visible')) {
        render();
  ...
  function attachIntoPresetManagerUI() {
    const addButton = () => {
      ...
      if ($('#preset-manager-group-btn').length > 0) return;
      const btn = $(
        '<button id="preset-manager-group-btn" style="margin-left: 10px; padding:6px 12px; background-color:#f4c78e; border:none; border-radius:6px; color:#3a2c2c; cursor:pointer; font-weight:bold;">分组管理</button>',
      );
      btn.on('click', () => (window as unknown as { PresetGroups_openUI?: () => void }).PresetGroups_openUI?.());
      host.append(btn);
    };
    addButton();
