...
declare global {
  interface Window {
    miaomiaoPromptGroupingLoaded?: boolean;
  }
}
...
import { generateUUID } from './初始化和配置';
...
// 显示分组选择弹窗
async function showGroupSelectionPopup(selectedPrompts: string[], existingGroups: PromptGroup[]): Promise<void> {
  ...
  let selectedGroup: PromptGroup | null = null;
  ...
  setTimeout(() => {
    ...
    console.log(
      '现有分组列表:',
      existingGroups.map(g => ({ id: g.id, name: g.name, type: typeof g.id })),
    );
...
// 将条目从分组中移除
async function removePromptsFromGroups(selectedPrompts: string[], existingGroups: PromptGroup[]): Promise<void> {
  selectedPrompts.forEach(promptId => {
...
// 显示解散分组弹窗
async function showDissolveGroupPopup(existingGroups: PromptGroup[]): Promise<void> {
  ...
  $('#group-selection-confirm').on('click', () => {
    ...
    selectedGroupNames.forEach(groupName => {
      ...
      if (groupIndex > -1) {
        ...
        // 递归处理子分组
        const processSubGroups = (subGroups: PromptGroup[]) => {
          subGroups.forEach(subGroup => {
            subGroup.promptIds.forEach(promptId => {
              const item = $(`.prompt-item[data-prompt-id="${promptId}"]`);
              item.css('background-color', '#fff');
