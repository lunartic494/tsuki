                ...
                for (const presetName of uniquePresetNames) {
                    if (TavernHelper.getPresetNames().includes(presetName)) {
                        ...
                        if (presetData) {
                            ...
                            presetData.name = actualPresetName;
...
async function saveCurrentConfig() {
    ...
    // 获取当前预设名称（用于显示和向后兼容）
    let currentPresetName = await (0,___WEBPACK_IMPORTED_MODULE_4__/* .getPresetNameByIdentifier */ .nx)(identifierId);
    if (!currentPresetName) {
...
async function exportConfig(configId) {
    try {
        ...
        if (groupingPresetName) {
            const groupingConfig = await (0,___WEBPACK_IMPORTED_MODULE_1__/* .exportPresetGrouping */ .pM)(groupingPresetName);
            if (groupingConfig) {
            ...
            if (groupingToImport && Array.isArray(groupingToImport) && groupingToImport.length > 0) {
                if (configToImport.presetName) {
                    try {
                        ...
                        await (0,___WEBPACK_IMPORTED_MODULE_1__/* .importPresetGrouping */ .q$)(configToImport.presetName, groupingToImport);
